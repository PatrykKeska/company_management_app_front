...
export const SingleFinalizedModal = (props: Props) => {
    ...
    return (
        <Wrapper>
            ...
            <StyledModal>
                <Button small={true} onClick={props.onClick}>Back</Button>

                {props.items.length <= 0 && (<><Paragraph>This plase has no items yet!</Paragraph>
                    <Paragraph>Please use inventory tab to add some</Paragraph></>)}
                {props.items.length > 0 && <Paragraph>Total costs of this Place:{SummaryCalc(props.items)}$</Paragraph>}
