// After build command this path gonna get /api
// export const apiURL = process.env.REACT_APP_API_URL ?? 'http://localhost:3001'
// if your api use different path please change it.
export const fileApi = 'https://patrykkeska.networkmanager.info/'
// export const fileApi = 'https://localhost:3001/files/'
// export const fileApi = 'https://kendziior4.usermd.net/api/files/'
// export const fetchFile = 'https://kendziior4.usermd.net/api/file-transfer'

// for localhost usage please use this version below !
export const apiURL = 'https://patrykkeska.networkmanager.info/api'
